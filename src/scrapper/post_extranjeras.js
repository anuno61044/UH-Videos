...
const procesarURLs = (rutaArchivo, audencies, clasification, countries, language) => {
    ...
    const lector = readline.createInterface({
        ...
        lector.on('line', (linea) => {
            // https://visuales.uclv.cu/Peliculas/Extranjeras/18xxs/1892/1892_Pauvre%20Pierrot%20(Emile%20Reynaud,/-foto--
            
            let line = linea.split('-foto-')

            if (line[0] != 'https://visuales.uclv.cu//Peliculas/Extranjeras/') {
                
                year = NaN
                let arr = line[0].split('/')
                let title = ''

                if(getLast(arr[arr.length-2].split('(')).length == 5) {
                    year = getLast(arr[arr.length-2].split('(')).split(')')[0]
...
procesarURLs('./Extranjeras.txt', 'adultos', 'pelicula', 'extranjera', 'ingles');
