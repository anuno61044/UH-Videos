...
def collaborative_filtering_train_set(user_id, train_items):
    """
    Realiza un filtrado colaborativo basado en la similitud de usuarios utilizando solo el conjunto de entrenamiento.
    ...
    - trace (dict): Un diccionario que contiene las puntuaciones de similitud del usuario, las calificaciones ponderadas, y la matriz de calificaciones de usuario-película.
    """
    # Obtener las calificaciones de solo el conjunto de entrenamiento
    ratings = Rating.objects.filter(movie_id__in=train_items)
    users = User.objects.all()
    movies = Movie.objects.all()

    # Crear una matriz de usuarios contra películas en el conjunto de entrenamiento, llena de ceros inicialmente
    user_movie_matrix = np.zeros((len(users), len(movies)))

    # Poblar la matriz con las calificaciones del conjunto de entrenamiento
    for rating in ratings:
        user_idx = list(users).index(rating.user)  # Encontrar el índice del usuario
        movie_idx = list(movies).index(rating.movie)  # Encontrar el índice de la película en el conjunto de entrenamiento
        user_movie_matrix[user_idx][movie_idx] = rating.score  # Asignar la calificación a la matriz
    ...
    # Calcular la similitud entre el usuario objetivo y los demás usuarios en el conjunto de entrenamiento
    for i in range(users.count()):
        if i != user_idx:
            common_ratings = (user_movie_matrix[user_idx] > 0) & (user_movie_matrix[i] > 0)
            if np.any(common_ratings):  # Asegurarse de que haya calificaciones comunes
                user_similarity[i] = cosine_similarity(
                    [user_movie_matrix[user_idx][common_ratings]],
                    [user_movie_matrix[i][common_ratings]]
                )[0, 0]
    ...
    min_rating = weighted_ratings.min()
    max_rating = weighted_ratings.max()
    normalized_ratings = (weighted_ratings - min_rating) / (max_rating - min_rating) if max_rating > min_rating else weighted_ratings

    # Guardar las trazas para seguimiento y explicabilidad
    trace = {
        "user_similarity_scores": user_similarity,  # Similitud del usuario con otros usuarios
        "weighted_ratings": normalized_ratings,  # Calificaciones ponderadas normalizadas
        "rating_matrix": user_movie_matrix  # Calificaciones de los usuarios a las películas en el conjunto de entrenamiento
    }
